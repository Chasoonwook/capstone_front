...
const normalizeTrack = (raw: any, idx: number): Track | null => {
  ...
  return { id: raw?.id ?? raw?.music_id ?? idx, title, artist, audioUrl, coverUrl, duration, spotify_track_id }
}
...
export default function RecommendClient() {
  ...
  const loadCurrentTrack = useCallback(
    async (autoplay = false) => {
      ...
      if (currentTrack.spotify_track_id) {
        // duration은 SDK state에서 가져오되, 초 단위로 반영
  ...
  const goToIndex = async (nextIndex: number) => {
    ...
    if (t?.spotify_track_id && sp.ready) {
      try { await sp.playUris([`spotify:track:${t.spotify_track_id}`]); setIsPlaying(true) }
      catch { setIsPlaying(false) }
    }
  }
  ...
  const onSeekCommit = async (value: number[]) => {
    ...
    if (t?.spotify_track_id) {
      ...
      const audio = audioRef.current
      if (audio) { audio.currentTime = v; setCurrentTime(v) }
    }
    setTimeout(() => setSeeking(false), 150)
  }

  const formatTime = (s: number) => {
    const m = Math.floor(s / 60), t = Math.floor(s % 60)
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-neutral-900 to-black flex items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="relative flex items-center mb-6 text-white">
          <Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={() => router.push("/?from=player")} title="메인으로">
          ...
          <div className="ml-auto flex items-center gap-2">
            {!sp.ready && (
              <a className="text-xs text-white/70 underline"
        ...
        <div className="mb-8">
          <div
            ...
            onClick={() => setShowPlaylist(true)} role="button" aria-label="재생목록 열기">
        ...
        {/* 곡 정보 */}
          <div className="flex items-start justify-between text-white mb-6">
            ...
            <div className="flex gap-2 flex-shrink-0">
              <Button variant="ghost" size="icon" onClick={toggleLike}
                className={cn("text-white hover:bg-white/10", currentTrack && likedTracks.has(currentTrack.id) && "text-red-500")}
                title="좋아요">
                <Heart className={cn("w-6 h-6", currentTrack && likedTracks.has(currentTrack.id) && "fill-red-500")} />
              </Button>
              <Button variant="ghost" size="icon" onClick={toggleDislike}
                className={cn("text-white hover:bg-white/10", currentTrack && dislikedTracks.has(currentTrack.id) && "text-blue-400")}
                title="별로예요">
                <ThumbsDown className={cn("w-6 h-6", currentTrack && dislikedTracks.has(currentTrack.id) && "fill-blue-400")} />
              </Button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="icon" onClick={handleUploadClick} className="text-white hover:bg-white/10 w-12 h-12" title="업로드">
                <Upload className="w-6 h-6" />
              </Button>
              <Button variant="ghost" size="icon" onClick={handlePrevious} className="text-white hover:bg-white/10 w-12 h-12" title="이전">
        ...
        {/* 핵심: 트랙 바뀔 때 오디오 엘리먼트도 리셋되도록 key 부여 */}
        <audio key={currentTrackIndex} ref={audioRef} preload="metadata" />
      </div>
      ...
      >
        <div className="p-6 text-white">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="icon" onClick={() => setShowPlaylist(false)} className="text-white hover:bg-white/10" title="닫기">
              ✕
            </Button>
          </div>
