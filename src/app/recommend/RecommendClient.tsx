...
import {
  ...
  MoreVertical, Heart, ThumbsDown, ListMusic, Upload,
} from "lucide-react";
...
const normalizeTrack = (raw: any, idx: number): Track | null => {
  ...
  else if (typeof raw?.preview_duration === "number") duration = raw.preview_duration / 1000;
...
/** 추천목록을 한 번에 커버/미리듣기/URI 보강(백엔드 batch 사용; 없어도 정상동작) */
async function prefetchCoversAndUris(list: Track[]): Promise<Track[]> {
  ...
  try {
    ...
    const map = new Map(items.map((it) => [
      it.key || `${norm(it.title)} - ${norm(it.artist)}`, it,
    ]));
    ...
    return list.map((t) => {
      ...
      return {
        ...
        duration: t.duration || hitDurationSec || null,
...
export default function RecommendClient() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!onceMountedRef.current) onceMountedRef.current = true;
      else if (lastQueueSigRef.current && playlist.length > 0) return;
      ...
      try {
        ...
        if (data && (data.main_songs || data.sub_songs || data.preferred_songs)) {
          ...
          list = all.map((r, i) => normalizeTrack(r, i)).filter((t): t is Track => t !== null);
        }

        const enhanced = await prefetchCoversAndUris(list);

        const sig = JSON.stringify(
          enhanced.map((t) => [t.id, t.spotify_track_id, !!t.audioUrl, !!t.spotify_uri]).concat([enhanced.length]),
        );
    ...
    run();
  }, [photoId, setQueueFromRecommend, playlist.length]);
  ...
  /** ✅ 미리듣기 없더라도 spotify_uri 있으면 선택 가능 */
  ...
  const artUrl = current?.coverUrl || analyzedPhotoUrl || "/placeholder.svg";
        ...
        <div className="mb-8">
          ...
          <div className="flex items-start justify-between text-white mb-6">
            ...
            <div className="flex gap-2 flex-shrink-0">
              <Button
                ...
                disabled={!current}
              >
                <Heart className={cn("w-6 h-6", current && likedTracks.has(current.id) && "fill-red-500")} />
              </Button>
              <Button
                variant="ghost"
                size="icon"
              ...
              >
                <ThumbsDown
                  className={cn("w-6 h-6", current && dislikedTracks.has(current.id) && "fill-blue-400")}
                />
              </Button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <Button
              ...
              /** ✅ 미리듣기 없어도 spotify_uri 있으면 재생 허용 */
              disabled={!current || (!current.audioUrl && !current.spotify_uri)}
            >
          ...
          <div className="overflow-y-auto" style={{ maxHeight: "calc(70vh - 100px)" }}>
            ...
            {!loading &&
              playlist.map((track, index) => {
                const playable = !!(track.audioUrl || track.spotify_uri); // ✅
                return (
                  <button
                    key={`${track.id}-${index}`}
                    ...
                    disabled={!playable} // ✅ 미리듣기 없더라도 spotify_uri 있으면 활성화
                  >
                    <img
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-white/60 truncate">
                        ...
                        <span className="ml-2 text-xs text-white/50">
                          {track.audioUrl ? "Preview" : track.spotify_uri ? "Spotify" : "재생 불가"}
                        </span>
                      </p>
                    </div>
