...
export function useSpotifyPlayer() {
    ...
    const [state, setState] = useState<WebPlaybackStateLite>({ position: 0, duration: 0, paused: true, trackUri: null });
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            player.addListener("ready", (ev) => { deviceIdRef.current = ev.device_id; setDeviceId(ev.device_id); setReady(true); });
    ...
    const activate = useCallback(async () => { await playerRef.current?.activateElement?.(); }, []);
    ...
    const resume = useCallback(async () => { await genericApiCall("/api/spotify/play", { method: "PUT", body: JSON.stringify({ device_id: deviceIdRef.current }) }); }, [genericApiCall]);
    const pause = useCallback(async () => { await genericApiCall("/api/spotify/pause", { method: "PUT", body: '{}' }); }, [genericApiCall]);
    const next = useCallback(async () => { await genericApiCall("/api/spotify/next", { method: "POST", body: '{}' }); }, [genericApiCall]);
    const prev = useCallback(async () => { await genericApiCall("/api/spotify/previous", { method: "POST", body: '{}' }); }, [genericApiCall]);
    const seek = useCallback(async (positionMs: number) => { await playerRef.current?.seek(Math.max(0, Math.floor(positionMs))); }, []);
